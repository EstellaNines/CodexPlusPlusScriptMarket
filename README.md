...
| Codex Main Transparency | 0.2.4 | 修复本地大图导入后 data URL 无法通过 CSS 变量显示的问题，改用 blob object URL 应用背景；本地背景图上限 32 MiB，并用 IndexedDB 保存导入图片。 | https://github.com/EstellaNines/CodexPlusPlusScriptMarket |
...
- Codex Main Transparency 的右下角入口可展开“透明与背景”面板，支持 background image URL、本地 `png/jpeg/webp/gif` 导入、背景透明度、铺满/完整/拉伸、背景模糊与自定义 shortcut；背景层位于页面画布之上、Codex 内容之下，并兼容早于 body 的注入时序；本地导入会存入 IndexedDB 并以 blob object URL 显示，本地导入上限为 32 MiB，默认背景开关快捷键为 `Alt+B`。
