...
| Codex Main Transparency | 0.2.1 | 右下角透明与背景设置面板：100% 仍为主界面 fully transparent；本地背景图上限扩至 32 MiB，并用 IndexedDB 保存导入图片，避免撑爆 `localStorage`。 | https://github.com/EstellaNines/CodexPlusPlusScriptMarket |
...
- Codex Main Transparency 的右下角入口可展开“透明与背景”面板，支持 background image URL、本地 `png/jpeg/webp/gif` 导入、背景透明度、铺满/完整/拉伸、背景模糊与自定义 shortcut；本地导入上限为 32 MiB，默认背景开关快捷键为 `Alt+B`。
